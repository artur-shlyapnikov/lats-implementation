...
def generate_with_accumulated_context(
    ...
    if strategy == "reflexion" and (
        prev_func_impl is None
        or accumulated_feedback is None
        or accumulated_reflection is None
    ):
        raise ValueError(
            f"Invalid arguments: given `strategy=reflexion` but `prev_func_impl`, `feedback`, or `self_reflection` is None"
        )
    ...
    accumulated_context = "\n\n".join(
        [
            ...
            for i, (impl, feedback, reflection) in enumerate(
                zip(prev_func_impl, accumulated_feedback, accumulated_reflection)
            )
        ]
    )
    ...
    if model.is_chat:
        if strategy == "reflexion":
            ...
            for impl, feedback, reflection in zip(
                prev_func_impl, accumulated_feedback, accumulated_reflection
            ):
            ...
            messages.append(
                Message(role="user", content=f"[improved impl]:\n{func_sig}")
            )
